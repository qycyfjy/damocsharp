...
public sealed class DmSoft
{
    ...
    public (int X, int Y) ClientToScreen(int hwnd, int x, int y)
    {
    ...
    public (int X, int Y) ScreenToClient(int hwnd, int x, int y)
    {
        var point = new POINT(x, y);
    ...
    [DllImport("DmReg.dll", CharSet = CharSet.Ansi)]
    public static extern int SetDllPathA(string path, int mode);
    [DllImport("kernel32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static extern IntPtr GetModuleHandle([MarshalAs(UnmanagedType.LPWStr)] string lpModuleName);

    public static DmSoft LoadAndUnlock()
    {
        SetDllPathA(@"dm.dll", 0);
        var damo = new Damo.DmSoft();
        var handle = GetModuleHandle("dm.dll");
        unsafe
        {
    ...
    public string GetInfo(string cmd, string param)
    {
        ...
        result = obj.InvokeMember("GetInfo", BindingFlags.InvokeMethod, null, obj_object, args);
        return result.ToString();
    }
    ...
    public string AsmCode(int base_addr)
    {
        ...
        result = obj.InvokeMember("AsmCode", BindingFlags.InvokeMethod, null, obj_object, args);
        return result.ToString();
    }
